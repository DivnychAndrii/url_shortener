...
class TestUrlManager:
    ...
    def test_create_short_url_hash_no_object_found(
            self,
    ...
    @patch('source.managers.url_manager.generate_random_hash')
    def test_create_short_url_hash_object_already_exist(
            self,
            generate_random_hash_mock: MagicMock,
            get_model_object_mock: MagicMock,
        ...
        result = test_manager.get_or_create_short_url_hash(
            url=target_object.original_url)

        assert result == target_object
        get_model_object_mock.assert_called_once_with(
            {'original_url': target_object.original_url})
