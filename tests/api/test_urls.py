...
class TestGenerateUrl:
    ...
    def test_generate_endpoint_integration(self, client: 'TestClient'):
        ...
        assert all(
            item in response.json() for item in ['short_url', 'clicks_count']
        )
    ...
    def test_generate_endpoint_schema_success(
            self,
        ...
        get_or_create_short_url_hash_mock.return_value = choice(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {'short_url': test_short_link,
                                   'clicks_count': random_short_link_obj.count}
    ...
    def test_generate_endpoint_schema_fail(self, client: 'TestClient',
                                           key: str) -> None:
        test_invalid_body = {
...
class TestRedirectUrl:
    ...
    def test_redirect_endpoint_schema_success(
            self,
        ...
        response = client.get(
            self.endpoint.format(url_key=valid_url_object.hash_key)
        )

        assert (next(iter(response.history)).status_code ==
